...
public interface FeedRepository extends JpaRepository<Feed, Long>, JpaSpecificationExecutor<Feed> {
    ...
    @Query("select f from Feed f where f.tenant.id = :tenantId and (f.deleted = false or f.deleted is null) order by f.createdAt desc")
    List<Feed> findAllByOrderByCreatedAtDesc(@Param("tenantId") Long tenantId, Pageable pageable);
